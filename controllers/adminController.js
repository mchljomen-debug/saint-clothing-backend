...
const getOrderDate=(order)=>new Date(order.date||order.createdAt||new Date());

const filterOrdersByBranchFromItems=(orders,branch)=>{
  return orders.filter((order)=>(order.items||[]).some((item)=>(item.branch||"")===branch));
};
...
const uploadEmployeeFile=async(file,folder)=>{
  ...
  if(!url){
    throw new Error(`Cloudinary upload failed for ${file.originalname||"employee file"}`);
  }
...
const buildDashboardData=async(user)=>{
  ...
  const employees=isAdmin
    ?await employeeModel.find({isDeleted:{$ne:true}}).sort({createdAt:-1})
    :await employeeModel.find({branch:branchCode,isDeleted:{$ne:true}}).sort({createdAt:-1});
  ...
  const paidOrders=orders.filter((order)=>order.payment===true||order.payment==="true");

  const totalRevenue=paidOrders.reduce((sum,order)=>sum+toNumber(order.amount),0);
  ...
  const topProducts=Object.values(productSalesMap).sort((a,b)=>b.sold-a.sold).slice(0,8);
  ...
  for(let i=5;i>=0;i--){
    ...
    const revenue=monthlyOrders.reduce((sum,order)=>sum+toNumber(order.amount),0);
  ...
  if(isAdmin){
    ...
    orders.forEach((order)=>{
      ...
      (order.items||[]).forEach((item)=>{
        ...
        branchSummaryMap[code].revenue+=toNumber(item.price)*toNumber(item.quantity);
...
export const adminLogin=async(req,res)=>{
  try{
    ...
    if(!["admin","manager","staff"].includes(employee.role)){
      return res.status(403).json({
        success:false,
        message:"This employee account does not have admin panel access."
      });
    }

    if(employee.role!=="admin"&&!employee.branch){
      return res.status(403).json({
        success:false,
        message:"This employee account has no assigned branch."
      });
    }
    ...
    await addLog({
      action:"ADMIN_LOGIN",
      message:`Admin panel login: ${employee.name} (${userRole})`,
      user:employee.name,
...
export const exportDashboardExcel=async(req,res)=>{
  try{
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=dashboard-${req.user.role==="admin"?"admin":req.user.branch}.xlsx`
    );
...
export const createEmployee=async(req,res)=>{
  try{
    ...
    if(deletedExisting){
      return res.status(400).json({
        success:false,
        message:"Employee email exists in trash. Restore or permanently delete it first."
      });
    }
    ...
    if(pictureFile?.buffer){
      pictureUrl=await uploadEmployeeFile(
        pictureFile,
        "saint-clothing/employees"
      );
    }

    if(resumeFile?.buffer){
      resumeUrl=await uploadEmployeeFile(
        resumeFile,
        "saint-clothing/employees/resumes"
      );
    }
    ...
    return res.json({
      ...
      employee:sanitizeEmployee(newEmployee)
    });
  }catch(err){
...
export const updateEmployee=async(req,res)=>{
  try{
    ...
    if(pictureFile?.buffer){
      employee.picture=await uploadEmployeeFile(
        pictureFile,
        "saint-clothing/employees"
      );
    }
    ...
    if(resumeFile?.buffer){
      employee.resume=await uploadEmployeeFile(
        resumeFile,
        "saint-clothing/employees/resumes"
      );
    }
