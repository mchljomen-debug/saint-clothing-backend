...
const updateProduct=async(req,res)=>{
  try{
    ...
    if(req.body.colorHex!==undefined){
      updateData.colorHex=req.body.colorHex;
    }
    ...
    if(req.body.outfitPosition!==undefined){
      updateData.outfitPosition=normalizeOutfitPosition(
    ...
    if(req.body.outfitBounds!==undefined){
      updateData.outfitBounds=normalizeOutfitBounds(
    ...
    if(req.body.styleVibe!==undefined){
      updateData.styleVibe=req.body.styleVibe||existingProduct.styleVibe||"Streetwear";
    }
...
const updateStock=async(req,res)=>{
  try{
    ...
    const hasActualAdd=SIZE_ORDER.some(
      (size)=>getSizeQty(addStockObj,size)>0
    );

    const hasPreorderAdd=SIZE_ORDER.some(
      (size)=>getSizeQty(addPreorderObj,size)>0
    );
