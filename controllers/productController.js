...
import uploadBufferToCloudinary,{uploadProductImageToCloudinary}from"../utils/cloudinaryUpload.js";
...
if(!userBranch)return{branch:"__NO_BRANCH__"};
...
const foundKey=Object.keys(plain).find((key)=>String(key).trim().toUpperCase()===target);
...
const formatProductName=(product)=>`${product?.name||"Unknown Product"}${product?.sku?` (${product.sku})`:""}`;

const resolveBranchCode=async(req,requestedBranch)=>{
if(!isAdmin(req)){
const userBranch=normalizeBranchCode(req.user?.branch);

if(!userBranch)throw new Error("No branch assigned to this account");
...
if(!normalizedRequested)throw new Error("Branch is required");
if(normalizedRequested==="all")return"all";
...
if(!existingBranch)throw new Error("Selected branch is invalid or inactive");

return existingBranch.code;
};

const uploadSingleIfExists=async(file,folder,removeBackground=false)=>{
...
const foundKey=Array.from(mapLike.keys()).find((item)=>String(item).trim().toUpperCase()===sizeKey);
...
const normalizeOutfitPosition=(value,fallback={})=>{
const parsed=parseObjectField(value,fallback);
...
scale:Number.isFinite(scale)?scale:Number(fallback?.scale??1),
width:Number.isFinite(width)?width:Number(fallback?.width??0),
height:Number.isFinite(height)?height:Number(fallback?.height??0)
};
};

const normalizeOutfitBounds=(value,fallback={})=>{
const parsed=parseObjectField(value,fallback);

let left=Number(parsed?.left);
let top=Number(parsed?.top);
let right=Number(parsed?.right);
let bottom=Number(parsed?.bottom);

if(!Number.isFinite(left))left=Number(fallback?.left??0);
if(!Number.isFinite(top))top=Number(fallback?.top??0);
if(!Number.isFinite(right))right=Number(fallback?.right??1);
if(!Number.isFinite(bottom))bottom=Number(fallback?.bottom??1);

left=Math.max(0,Math.min(1,left));
top=Math.max(0,Math.min(1,top));
right=Math.max(left,Math.min(1,right));
bottom=Math.max(top,Math.min(1,bottom));
...
if(!preorderEnabled||!preorderAutoGenerate)return nextPreorderStock;
...
if(logs.length>0)await inventoryLogModel.insertMany(logs);
};
...
const url=await uploadSingleIfExists(req.files.image1[0],"saint-clothing/products",true);
if(url)images.push(url);
}

if(req.files?.image2?.[0]){
const url=await uploadSingleIfExists(req.files.image2[0],"saint-clothing/products",true);
if(url)images.push(url);
}

if(req.files?.image3?.[0]){
const url=await uploadSingleIfExists(req.files.image3[0],"saint-clothing/products",true);
if(url)images.push(url);
}

if(req.files?.image4?.[0]){
const url=await uploadSingleIfExists(req.files.image4[0],"saint-clothing/products",true);
if(url)images.push(url);
}

const outfitImage=req.files?.outfitImage?.[0]?await uploadSingleIfExists(req.files.outfitImage[0],"saint-clothing/outfits",true):"";
const model3d=req.files?.model3d?.[0]?await uploadModelIfExists(req.files.model3d[0]):"";
const sizeChartImage=req.files?.sizeChartImage?.[0]?await uploadSingleIfExists(req.files.sizeChartImage[0],"saint-clothing/size-charts"):"";
...
const finalPreorderThreshold=Math.max(0,parseNumber(preorderThreshold,5));
const finalPreorderAutoGenerate=parseBoolean(preorderAutoGenerate,true);
const finalPreorderAutoStock=Math.max(0,parseNumber(preorderAutoStock,20));
...
const finalOutfitPosition=normalizeOutfitPosition(outfitPosition,{
x:0,
y:0,
scale:1,
width:0,
...
const finalOutfitBounds=normalizeOutfitBounds(outfitBounds,{
left:0,
top:0,
right:1,
bottom:1
...
preorderAutoGenerate:finalPreorderAutoGenerate,
preorderAutoStock:finalPreorderAutoStock,
preorderRestockDate:preorderRestockDate?new Date(preorderRestockDate):null,
preorderNote:preorderNote||"",
colors:parseArrayField(colors,[]),
images,
outfitImage,
outfitPosition:finalOutfitPosition,
outfitBounds:finalOutfitBounds,
sizeChartImage,
model3d,
onSale:finalOnSale,
...
recommendationSection:normalizeRecommendationSection(recommendationSection),
...
if(!id||id==="undefined")return res.status(400).json({success:false,message:"Product ID is missing"});
...
if(req.body.groupCode!==undefined)updateData.groupCode=String(req.body.groupCode).trim();
if(req.body.color!==undefined)updateData.color=req.body.color;
if(req.body.colorHex!==undefined)updateData.colorHex=req.body.colorHex;
if(req.body.description!==undefined)updateData.description=req.body.description;
if(req.body.category!==undefined)updateData.category=req.body.category;
if(req.body.price!==undefined)updateData.price=parseNumber(req.body.price,existingProduct.price);

if(req.files?.sizeChartImage?.[0]){
updateData.sizeChartImage=await uploadSingleIfExists(req.files.sizeChartImage[0],"saint-clothing/size-charts");
}

if(req.files?.outfitImage?.[0]){
updateData.outfitImage=await uploadSingleIfExists(req.files.outfitImage[0],"saint-clothing/outfits",true);
}

if(req.body.outfitPosition!==undefined){
updateData.outfitPosition=normalizeOutfitPosition(req.body.outfitPosition,existingProduct.outfitPosition||{
x:0,
y:0,
scale:1,
width:0,
...
if(req.body.outfitBounds!==undefined){
updateData.outfitBounds=normalizeOutfitBounds(req.body.outfitBounds,existingProduct.outfitBounds||{
left:0,
top:0,
right:1,
bottom:1
...
const nextStock=req.body.stock!==undefined
?parseObjectField(req.body.stock,existingProduct.stock||{})
:parseObjectField(existingProduct.stock||{},{});

const nextPreorderEnabled=req.body.preorderEnabled!==undefined
?parseBoolean(req.body.preorderEnabled,existingProduct.preorderEnabled!==false)
:existingProduct.preorderEnabled!==false;

const nextPreorderThreshold=req.body.preorderThreshold!==undefined
?Math.max(0,parseNumber(req.body.preorderThreshold,existingProduct.preorderThreshold||5))
:Math.max(0,Number(existingProduct.preorderThreshold??5));

const nextPreorderAutoGenerate=req.body.preorderAutoGenerate!==undefined
?parseBoolean(req.body.preorderAutoGenerate,existingProduct.preorderAutoGenerate!==false)
:existingProduct.preorderAutoGenerate!==false;

const nextPreorderAutoStock=req.body.preorderAutoStock!==undefined
?Math.max(0,parseNumber(req.body.preorderAutoStock,existingProduct.preorderAutoStock||20))
:Math.max(0,Number(existingProduct.preorderAutoStock??20));

const nextPreorderStock=req.body.preorderStock!==undefined
...
updateData.preorderRestockDate=req.body.preorderRestockDate?new Date(req.body.preorderRestockDate):null;
}

if(req.body.preorderNote!==undefined)updateData.preorderNote=String(req.body.preorderNote||"");
...
updateData.bestseller=parseBoolean(req.body.bestseller,existingProduct.bestseller);
}

if(req.body.newArrival!==undefined){
updateData.newArrival=parseBoolean(req.body.newArrival,existingProduct.newArrival);
}

const nextOnSale=req.body.onSale!==undefined
?parseBoolean(req.body.onSale,existingProduct.onSale)
:existingProduct.onSale;

const nextSalePercent=req.body.salePercent!==undefined
...
if(req.body.fitType!==undefined)updateData.fitType=req.body.fitType||existingProduct.fitType||"Regular";
if(req.body.styleVibe!==undefined)updateData.styleVibe=req.body.styleVibe||existingProduct.styleVibe||"Streetwear";

if(req.body.recommendationSection!==undefined){
updateData.recommendationSection=normalizeRecommendationSection(req.body.recommendationSection);
}

if(req.body.styleTags!==undefined){
updateData.styleTags=parseArrayField(req.body.styleTags,existingProduct.styleTags||[]);
}

if(req.body.matchWith!==undefined){
updateData.matchWith=parseArrayField(req.body.matchWith,existingProduct.matchWith||[]);
}
...
const nextPreorderEnabled=preorderEnabled!==undefined
?parseBoolean(preorderEnabled,true)
:product.preorderEnabled!==false;

const nextPreorderThreshold=preorderThreshold!==undefined
?Math.max(0,parseNumber(preorderThreshold,5))
:Math.max(0,Number(product.preorderThreshold??5));

const nextPreorderAutoGenerate=preorderAutoGenerate!==undefined
?parseBoolean(preorderAutoGenerate,true)
:product.preorderAutoGenerate!==false;

const nextPreorderAutoStock=preorderAutoStock!==undefined
...
const metaChanged=
Boolean(product.preorderEnabled!==false)!==Boolean(nextPreorderEnabled)||
Number(product.preorderThreshold??5)!==Number(nextPreorderThreshold)||
Boolean(product.preorderAutoGenerate!==false)!==Boolean(nextPreorderAutoGenerate)||
Number(product.preorderAutoStock??20)!==Number(nextPreorderAutoStock)||
String(product.preorderRestockDate||"")!==String(preorderRestockDate||"")||
String(product.preorderNote||"")!==String(preorderNote||"");

if(!hasActualAdd&&!hasPreorderAdd&&!metaChanged){
return res.status(400).json({
success:false,
message:"Please enter stock quantity to add or change pre-order settings"
});
}
...
product.preorderRestockDate=preorderRestockDate?new Date(preorderRestockDate):null;
}

if(preorderNote!==undefined)product.preorderNote=String(preorderNote||"");
...
if(preorderMode){
const currentPreorderQty=getPreorderValue(product,sizeKey);
setMapValue(product,"preorderStock",sizeKey,currentPreorderQty-qty);
}else{
const currentQty=getStockValue(product,sizeKey);
...
await addLog({
action:preorderMode?"PRODUCT_PREORDER_STOCK_DEDUCTED":"PRODUCT_STOCK_DEDUCTED",
...
if(!deliveredOrder)return res.json({success:true,canReview:false});

const product=await Product.findById(id);

const alreadyReviewed=product?.reviews?.some((review)=>String(review.userId)===String(userId));
...
const userName=userData?.name?.trim()||(userData?.email?userData.email.split("@")[0]:"")||"Verified Buyer";
