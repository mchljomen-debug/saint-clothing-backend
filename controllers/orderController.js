...
const normalizeBranch = (branch) => String(branch || "branch1").trim() || "branch1";
...
const isPreorderMode = (product, sizeKey) => {
  ...
  return product?.preorderEnabled !== false && actualStock <= 0 && preorderStock > 0;
};

const getOrderBranches = (order) => {
  ...
  if (branches.size === 0 && order?.branch && order.branch !== "multiple") {
    branches.add(normalizeBranch(order.branch));
  }

  return [...branches];
};

const orderContainsBranch = (order, branch) => {
  const target = normalizeBranch(branch);
  return getOrderBranches(order).some((value) => normalizeBranch(value) === target);
};

const canManageOrder = (req, order) => {
  if (isAdmin(req)) return true;
  return orderContainsBranch(order, req.user?.branch);
};
...
const getPaymongoPaymentStatus = (payment = {}) => String(payment?.attributes?.status || payment?.status || "").trim().toLowerCase();
const getPaymongoPaymentIntentStatus = (paymentIntent = {}) => String(paymentIntent?.attributes?.status || paymentIntent?.status || "").trim().toLowerCase();
...
const validateAndNormalizeItems = async (items) => {
  ...
  for (const item of items) {
    ...
    const productBranch = normalizeBranch(product.branch);
    branches.add(productBranch);

    const sizeKey = String(item.size || "S").trim().toUpperCase();
    ...
    if (!preorderMode && actualStock < quantity) {
      const err = new Error(`Not enough stock for ${product.name} (${sizeKey}) at ${productBranch}`);
      err.statusCode = 400;
      throw err;
    }

    if (preorderMode && preorderStock < quantity) {
      const err = new Error(`Pre-order stock not enough for ${product.name} (${sizeKey}) at ${productBranch}`);
      err.statusCode = 400;
      throw err;
    }
  ...
  const orderBranches = [...branches];

  return {
    orderBranch: orderBranches.length === 1 ? orderBranches[0] : "multiple",
    orderBranches,
    normalizedItems
  };
};
...
const deductOrderStock = async (items) => {
  for (const item of items) {
    ...
    if (productBranch !== itemBranch) {
      const err = new Error(`Branch mismatch for ${product.name}. Expected ${itemBranch}, found ${productBranch}`);
      err.statusCode = 400;
      throw err;
    }
    ...
    const sizeKey = String(item.size || "S").trim().toUpperCase();
    const quantity = Number(item.quantity || 0);
    const preorderMode = !!item.isPreorder;

    if (preorderMode) {
      const availablePreorder = getPreorderValue(product, sizeKey);

      if (availablePreorder < quantity) {
        const err = new Error(`Pre-order stock issue for ${product.name} (${sizeKey}) at ${productBranch}`);
        err.statusCode = 400;
        throw err;
      }
      ...
      if (available < quantity) {
        const err = new Error(`Stock issue for ${product.name} (${sizeKey}) at ${productBranch}`);
        err.statusCode = 400;
        throw err;
      }
    ...
    await addLog({
      ...
      message: `${preorderMode ? "Pre-order stock" : "Stock"} deducted: ${product.name} (${sizeKey}) -${quantity} from ${productBranch}`,
      user: "System",
...
const restoreOrderStock = async (items) => {
  for (const item of items) {
    ...
    await addLog({
      ...
      message: `${item.isPreorder ? "Pre-order stock" : "Stock"} restored: ${product.name} (${sizeKey}) +${quantity} to ${productBranch}`,
      user: "System",
      entityId: product._id,
      entityType: "Product"
    });
...
const verifyPaymongoCheckout = async (order) => {
  if (!order?.paymongoCheckoutId || !process.env.PAYMONGO_SECRET_KEY) {
    return { paid: false, payment: null, paymentIntent: null };
  }

  try {
    const response = await axios.get(
      `https://api.paymongo.com/v1/checkout_sessions/${order.paymongoCheckoutId}`,
      { headers: getPaymongoAuthHeaders(), timeout: 20000 }
    );
    ...
    const paid = !!paidPayment || paymentIntentStatus === "paid" || paymentIntentStatus === "succeeded";
    ...
    return { paid, payment: paidPayment, paymentIntent };
  } catch (error) {
    console.error("PAYMONGO CHECKOUT VERIFICATION ERROR:", error.response?.data || error.message);
    return { paid: false, payment: null, paymentIntent: null };
  }
};
...
const placeOrder = async (req, res) => {
  try {
    ...
    if (!authUserId || String(authUserId) !== String(userId)) {
      return res.status(403).json({ success: false, message: "Unauthorized order request" });
    }
    ...
    const { orderBranch, orderBranches, normalizedItems } = await validateAndNormalizeItems(items);
    ...
    await addLog({
      ...
      message: hasPreorderItems
        ? `Pre-order placed: ${newOrder._id} via ${normalizedPaymentMethod} (${orderBranches.join(", ")})`
        : `Order placed: ${newOrder._id} via ${normalizedPaymentMethod} (${orderBranches.join(", ")})`,
...
const markOrderAsPaidFromPaymongo = async (order, paymentData = {}) => {
  ...
  if (order.payment === true || String(order.paymentStatus || "").toLowerCase() === "paid") return order;
  ...
  const paymentIntent = paymentData?.attributes?.payment_intent || paymentData?.attributes?.payment_intent_id || "";

  order.paymongoPaymentId = paymentData?.id || order.paymongoPaymentId || "";
  order.paymongoPaymentIntentId = typeof paymentIntent === "string" ? paymentIntent : paymentIntent?.id || order.paymongoPaymentIntentId || "";

  await order.save();

  const cartCleaned = await removeOrderItemsFromCart(order.userId, order.items);
...
const paymongoWebhook = async (req, res) => {
  try {
    ...
    const checkoutSessionId = eventData?.attributes?.checkout_session_id || eventData?.attributes?.checkout_session || "";
    const rawPaymentIntent = eventData?.attributes?.payment_intent_id || eventData?.attributes?.payment_intent || "";
    const paymentIntentId = typeof rawPaymentIntent === "string" ? rawPaymentIntent : rawPaymentIntent?.id || "";
    const metadataOrderId = eventData?.attributes?.metadata?.orderId || eventData?.attributes?.metadata?.order_id || "";

    let order = null;

    if (checkoutSessionId) order = await orderModel.findOne({ paymongoCheckoutId: String(checkoutSessionId) });
    if (!order && paymentIntentId) order = await orderModel.findOne({ paymongoPaymentIntentId: String(paymentIntentId) });
    if (!order && metadataOrderId) order = await orderModel.findById(metadataOrderId);
    if (!order && eventData?.id) order = await orderModel.findOne({ paymongoCheckoutId: String(eventData.id) });
...
const getPaymentStatus = async (req, res) => {
  try {
    ...
    let paid = order.payment === true || String(order.paymentStatus || "").toLowerCase() === "paid";

    if (!paid && normalizePaymentMethod(order.paymentMethod) === "PayMongo" && order.paymongoCheckoutId) {
...
const updateTrackingNumber = async (req, res) => {
  try {
    ...
    if (!cleanTracking) {
      return res.status(400).json({ success: false, message: "J&T tracking number is required" });
    }
    ...
    if (["Order Placed", "Packing"].includes(order.status)) order.status = "Shipped";
    ...
    return res.json({ success: true, message: "J&T tracking number updated", order });
  } catch (error) {
...
const submitPaymentProof = async (req, res) => {
  try {
    ...
    if (!req.file?.buffer) return res.status(400).json({ success: false, message: "Payment proof image is required" });
    ...
    if (!isManualPayment(normalizedPaymentMethod)) {
      return res.status(400).json({ success: false, message: "Only manual payment methods can submit payment proof" });
    }

    const uploadedProof = await uploadBufferToCloudinary(req.file.buffer, "saint-clothing/payment-proofs");
...
const approveManualPayment = async (req, res) => {
  try {
    ...
    if (!isManualPayment(method)) {
      return res.status(400).json({ success: false, message: "Only manual payment orders can be approved here" });
    }

    if (order.payment) return res.json({ success: true, message: "Payment already approved" });
...
const rejectManualPayment = async (req, res) => {
  try {
    ...
    if (!isManualPayment(method)) {
      return res.status(400).json({ success: false, message: "Only manual payment orders can be rejected here" });
    }
...
const allOrders = async (req, res) => {
  try {
    const filter = isAdmin(req) ? {} : {
      $or: [
        { "items.branch": normalizeBranch(req.user?.branch) },
        { branches: normalizeBranch(req.user?.branch) },
...
const userOrders = async (req, res) => {
  try {
    ...
    if (!finalUserId) return res.status(401).json({ success: false, message: "Unauthorized" });
...
const updateStatus = async (req, res) => {
  try {
    ...
    if (isOnlinePayment(method) && method !== "COD" && currentPaymentStatus !== "paid" && ["Packing", "Shipped", "Out for Delivery", "Delivered"].includes(normalized)) {
      return res.status(400).json({
        success: false,
    ...
    if (normalized === "Delivered") {
      await addLog({
        ...
        message: `Order completed and added to delivered order archive: ${order._id}`,
        user: getActorName(req, "Admin"),
        entityId: order._id,
        entityType: "Order"
      });
...
const receiveOrder = async (req, res) => {
  try {
    ...
    const order = await orderModel.findOne({ _id: orderId, userId: authUserId });
    ...
    const uploadedProof = await uploadBufferToCloudinary(req.file.buffer, "saint-clothing/delivery-proofs");
...
const cancelOrder = async (req, res) => {
  try {
    ...
    if (isOnlinePayment(order.paymentMethod)) order.paymentStatus = "failed";
